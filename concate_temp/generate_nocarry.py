...
if __name__ == "__main__":
    ...
    final_text_path_nc = 'data/nocarry.jsonl'
    template_file = 'data/temp_pair_comp_trans.csv'
    n_instances = 1

    instantiated_nc, instantiated_c, add_sub_both, _ = instantiate_nocarry_carry(empty_lf_path, destination_path_nc, destination_path_c, n_instances)
    temp_dict = load_temp_dict(template_file)
    templated_mwp = generate_mwp(instantiated_nc, temp_dict)
    final_mwp_nc = paraphrase_problems(templated_mwp, carry=True)

    with open(final_text_path_nc, 'w') as f:
        for mwp_dict in final_mwp_nc:
